import cv2
cascade_face = cv2.CascadeClassifier('D:\python\py\happy\haarcascade_frontalface_default.xml') 
cascade_eye = cv2.CascadeClassifier('D:\python\py\happy\haarcascade_eye.xml') 
cascade_smile = cv2.CascadeClassifier('D:\python\py\happy\haarcascade_smile.xml')

def detection(grayscale, img):
    face = cascade_face.detectMultiScale(grayscale, 1.3, 5)
    for (x_face, y_face, w_face, h_face) in face:
        cv2.rectangle(img, 
        (x_face, y_face), (x_face+w_face, y_face+h_face), (255, 130, 0), 2)
